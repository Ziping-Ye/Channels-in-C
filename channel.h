...
typedef struct {
    ...
    buffer_t* buffer;
    int closed; //1 if closed, o if open
    pthread_mutex_t bmutex; //mutex for buffer
    pthread_mutex_t lmutex; //mutex for linked_list
    sem_t sender;
    sem_t receiver;
    list_t* send_list; //linked_list to keep track of all selects that subscribe to send operation on this channel
    list_t* receive_list; //linked_list to keep track of all selects that subscribe to receive operation on this channel
} chan_t;
